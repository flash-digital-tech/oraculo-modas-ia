...
async def showLinks():
    ...
    with st.form(key='create_link_pagamento'):
        ...
        valeu = st.number_input("Valor do Pagamento", min_value=0.0, format="%.2f", key='valor')
        ...
        if submit_button:
            novo_link = LinkPagamento(name=name,value=valeu, billingType=billingType, chargeType=chargeType, description=description)
            import requests
