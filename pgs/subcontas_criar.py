...
async def showParceiro():
    ...
    incomeValue = st.number_input("Qual sua renda mensal ou faturamento:", min_value=0.0, format="%.2f", step=0.01)
    ...
    if st.button("Criar Subconta"):
        ...
        if not all([nome, email, cpf_cnpj, mobile_phone, incomeValue, address, number, province, city, neighborhood, postalCode]):
            ...
            birth_date_str = birthDate.strftime("%Y-%m-%d") if birthDate else None
